...
import { readBindings, readBoolean, readTerm } from './json.ts'

describe('@okikio/sparql JSON results', () => {
  it('preserves RDF literal datatype, language, and RDF 1.2 direction', () => {
    const value = readTerm({ type: 'literal', value: 'bonjour', 'xml:lang': 'fr', 'its:dir': 'ltr' })
  ...
  it('decodes SELECT and ASK result modes without JavaScript datatype coercion', () => {
    const rows = readBindings({
      head: { vars: ['price'] },
      results: { bindings: [{ price: { type: 'literal', value: '12.50', datatype: 'http://www.w3.org/2001/XMLSchema#decimal' } }] },
    })
    expect(rows).toHaveLength(1)
    expect(rows[0]?.get('price')?.termType).toBe('Literal')
    expect(readBoolean({ boolean: true })).toBe(true)
    expect(() => readBoolean({ boolean: 'true' })).toThrow()
  })
})
