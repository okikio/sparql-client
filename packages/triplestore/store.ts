...
import { FORMAT, generationName, parseCommit, parseFormat, writeRecord, type CommitType } from './format.ts'
...
export class Store implements AsyncDisposable {
   ...
   */
  async import(source: Iterable<Quad> | AsyncIterable<Quad>, options: ImportOptionsType = {}): Promise<this> {
  ...
  async #publish(commit: CommitType, segmentText: string, signal?: AbortSignal): Promise<void> {
    ...
    if (await this.#fs.exists(commitPath, signalOptions(signal))) {
      throw new StoreError('writer-conflict', `Commit generation ${commit.generation} already exists.`)
    }
  ...
  #assertWritable(): void {
    if (this.#closing || this.#closed) throw new StoreError('closed', 'Triplestore is closing or closed.')
  }
  ...
  static async open(fs: FileSystemType, options: OpenOptionsType = {}): Promise<Store> {
    ...
    const maxSegmentBytes = positive(options.maxSegmentBytes ?? DEFAULT_MAX_SEGMENT_BYTES, 'maxSegmentBytes')
    abort(options.signal)
    ...
    if (committedFailure !== undefined) {
      throw new StoreError('corrupt', 'No valid committed triplestore generation could be recovered.', {
...
export type StoreErrorKind = 'closed' | 'format' | 'corrupt' | 'writer-conflict'
...
type OperationType = { readonly kind: 'add' | 'delete'; readonly quad: Quad }

/** Creates or opens one persistent RDF store. */
...
async function recover(
  ...
  while (generation > 0) {
    ...
    if (commit.generation !== generation) throw new StoreError('corrupt', `Commit path generation ${generation} disagrees with record ${commit.generation}.`)
    const stem = generationName(generation)
    const expectedSegment = commit.mode === 'snapshot' ? `segments/${stem}.nq` : `segments/${stem}.delta.nq`
    if (commit.segment !== expectedSegment) throw new StoreError('corrupt', `Commit ${generation} references unexpected segment '${commit.segment}'.`)
  ...
  for (const commit of [...commits.values()].reverse()) {
    ...
    if (stat.kind !== 'file') throw new StoreError('corrupt', `Segment '${commit.segment}' is not a file.`)
    if (stat.size > maxSegmentBytes) throw new StoreError('corrupt', `Segment '${commit.segment}' exceeds maxSegmentBytes.`)
    const text = await fs.readText(path, signalOptions(signal))
    if (await checksum(text) !== commit.checksum) throw new StoreError('corrupt', `Segment checksum mismatch for '${commit.segment}'.`)
  ...
  if (dataset.size !== current.quadCount) {
    throw new StoreError('corrupt', `Recovered ${dataset.size} quads but commit ${head} records ${current.quadCount}.`)
  }
  return dataset
}
...
function operationKind(operations: readonly OperationType[]): OperationType['kind'] {
  ...
  for (const operation of operations) {
    if (operation.kind !== first.kind) throw new TypeError('Delta generation cannot mix add and delete operations.')
  }
  return first.kind
}
...
function normalizePath(value: string): string {
  ...
  if (parts.some((part) => part === '.' || part === '..')) throw new TypeError('Store path cannot contain dot segments.')
  return `/${parts.join('/')}`
}
...
function writeOptions(signal?: AbortSignal): { readonly signal?: AbortSignal; readonly parents: true } {
