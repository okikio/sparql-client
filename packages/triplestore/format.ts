...
export function parseFormat(text: string): FormatType {
  ...
  if (value.version !== FORMAT_VERSION || value.store !== '@okikio/triplestore' || value.segment !== SEGMENT_VERSION) {
...
export function parseCommit(text: string): CommitType {
  ...
  if (value.version !== FORMAT_VERSION) throw new TypeError(`Unsupported commit version '${String(value.version)}'.`)
  if (!isInteger(value.generation) || value.generation < 1) throw new TypeError('Commit generation must be a positive safe integer.')
  if (!isInteger(value.parent) || value.parent !== value.generation - 1) throw new TypeError('Commit parent must be the preceding generation.')
  if (value.mode !== 'delta' && value.mode !== 'snapshot') throw new TypeError(`Unknown commit mode '${String(value.mode)}'.`)
  if (typeof value.segment !== 'string' || !/^segments\/[0-9]{16}(?:\.delta)?\.nq$/.test(value.segment)) throw new TypeError('Commit segment path is invalid.')
  ...
  if (typeof value.checksum !== 'string' || !/^sha256:[0-9a-f]{64}$/.test(value.checksum)) throw new TypeError('Commit checksum is invalid.')
  if (!isInteger(value.quadCount) || value.quadCount < 0) throw new TypeError('Commit quadCount must be a non-negative safe integer.')
  return {
    version: FORMAT_VERSION,
...
export function generationName(generation: number): string {
  if (!isInteger(generation) || generation < 1) throw new RangeError('generation must be a positive safe integer.')
...
function parseRecord(text: string, label: string): Record<string, unknown> {
  ...
  if (typeof value !== 'object' || value === null || Array.isArray(value)) throw new TypeError(`Triplestore ${label} must be an object.`)
