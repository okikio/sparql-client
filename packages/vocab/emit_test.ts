...
describe('@okikio/vocab', () => {
  it('emits deterministic direct class term/type/schema exports', async () => {
    ...
    const model = await read([{ id: 'schema', quads: [quad(product, RDF_TYPE, RDFS_CLASS)] }])
    const source = emit(model, { vocabulary: 'Schema.org', namespace: 'https://schema.org/', prefix: 'schema' }).source
