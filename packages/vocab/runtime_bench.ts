...
const validate = (values: readonly unknown[]): number => {
  ...
  for (const value of values) {
    ...
    if (result instanceof Promise) throw new Error('Generated bootstrap schema unexpectedly became async.')
...
if (validate(invalid) !== PRODUCTS) throw new Error('Vocabulary runtime benchmark invalid-data oracle failed.')
