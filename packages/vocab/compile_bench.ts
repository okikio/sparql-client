...
import { namedNode, quad, type Quad } from '@okikio/rdf'
...
for (let index = 0; index < PROPERTY_COUNT; index++) {
  ...
  quads.push(quad(property, RDFS_DOMAIN, namedNode(`https://example.test/Class${index % CLASS_COUNT}`)))
...
if (!oracle.source.includes('export const Class499 =')) throw new Error('Vocabulary compiler benchmark oracle failed.')
