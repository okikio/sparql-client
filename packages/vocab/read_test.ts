...
describe('@okikio/vocab ontology adapter', () => {
  it('adds Schema.org domain/range aliases without changing the generic ontology reader', async () => {
    const source = `
    ...
    `
    const model = await read([{ id: 'schema-style', quads: parse(source) }])
  ...
  it('accepts additional vocabulary-specific relationship aliases', async () => {
    ...
    `
    const model = await read([{ id: 'custom', quads: parse(source) }], {
