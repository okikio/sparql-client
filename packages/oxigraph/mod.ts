 ...
 */
export function createClient(store: StoreType, options: ClientOptionsType = {}): Client {
  return {
    store,
    ...
    async queryBindings(query, queryOptions = {}) {
      ...
      if (!isIterable(result)) throw new TypeError('Oxigraph SELECT query did not return an iterable of bindings.')
    ...
    async queryQuads(query, queryOptions = {}) {
      prepare(queryOptions)
      const result = store.query(getQueryText(query), options.query)
      if (!isIterable(result)) throw new TypeError('Oxigraph graph query did not return an iterable of quads.')
      return quads(result)
    },
    ...
    async queryBoolean(query, queryOptions = {}) {
      prepare(queryOptions)
      const result = store.query(getQueryText(query), options.query)
      if (typeof result !== 'boolean') throw new TypeError('Oxigraph ASK query did not return a boolean.')
      return result
    },
...
function prepare(options: QueryOptionsType): void {
  ...
  if (options.timeoutMs !== undefined && options.timeoutMs !== null && options.timeoutMs > 0) {
    throw new TypeError('Oxigraph synchronous Store queries cannot honor timeoutMs. Run the store in an owned Worker when interruptibility is required.')
  }
}
...
async function* bindings(values: Iterable<unknown>): AsyncGenerator<BindingType> {
  for (const value of values) {
    ...
    for (const [name, term] of value) {
      if (typeof name !== 'string') throw new TypeError('Oxigraph binding variable name is not a string.')
...
function isTerm(value: unknown): value is Term {
  ...
  return typeof record.termType === 'string' && typeof record.value === 'string' && typeof record.equals === 'function'
}

/** Returns whether the supplied value satisfies the quad contract. */
function isQuad(value: unknown): value is Quad {
  return isTerm(value) && value.termType === 'Quad' && 'subject' in value && 'predicate' in value && 'object' in value && 'graph' in value
}
